...
export async function getMovieWatchProviders(movieId, movieTitle = null) {
  try {
    ...
    // DEBUG: Ver toda a resposta da API
    console.log("🔍 Watch Providers Response completa (Movie):", data);
    console.log("🔍 Results:", data.results);
    
    // Verificar todos os países disponíveis
    if (data.results) {
      console.log("🔍 Países disponíveis:", Object.keys(data.results));
      Object.keys(data.results).forEach(country => {
        const providers = data.results[country];
        console.log(`🔍 ${country}:`, providers);
        if (providers.flatrate) {
          console.log(`  - Flatrate providers:`, providers.flatrate);
          const netflix = providers.flatrate.find(p => p.provider_id === 8);
          if (netflix) {
            console.log(`  ✅ Netflix encontrado em ${country}!`);
          }
        }
      });
    }
    ...
    console.log("🔍 PT Providers:", ptProviders);
    if (ptProviders?.flatrate) {
      console.log("🔍 PT Flatrate:", ptProviders.flatrate);
      const netflix = ptProviders.flatrate.find(p => p.provider_id === 8);
      console.log("🔍 Netflix em PT:", netflix);
      if (netflix) {
        const searchQuery = movieTitle ? encodeURIComponent(movieTitle) : '';
        console.log("✅ Netflix encontrado em PT!");
        return {
          hasNetflix: true,
    ...
    console.log("🔍 US Providers:", usProviders);
    if (usProviders?.flatrate) {
      console.log("🔍 US Flatrate:", usProviders.flatrate);
      const netflix = usProviders.flatrate.find(p => p.provider_id === 8);
      console.log("🔍 Netflix em US:", netflix);
      if (netflix) {
        const searchQuery = movieTitle ? encodeURIComponent(movieTitle) : '';
        console.log("✅ Netflix encontrado em US!");
        return {
          hasNetflix: true,
          netflixUrl: `https://www.netflix.com/search?q=${searchQuery}`
        };
    ...
    console.log("❌ Netflix não encontrado em PT nem US");
...
export async function getSeriesWatchProviders(seriesId, serieTitle = null) {
  try {
    ...
    // DEBUG: Ver toda a resposta da API
    console.log("🔍 Watch Providers Response completa:", data);
    console.log("🔍 Results:", data.results);
    ...
    if (data.results) {
      ...
      Object.keys(data.results).forEach(country => {
        ...
        if (providers.buy) {
          console.log(`  - Buy providers:`, providers.buy);
        }
        if (providers.rent) {
          console.log(`  - Rent providers:`, providers.rent);
        }
      });
    }
